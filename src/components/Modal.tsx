...
export default function BasicModal() {
  ...
  const formSchema = yup.object().shape({
    name: yup.string().min(3, 'min 3 chars').max(20, '20 charecters or less').required('required'),
    email: yup.string().email('Invalid email').required('Email is required'),
    number: yup.number().integer('number must be an integer').positive('number must be positive').required('number is required'),
  ...
  const handleSubmit = (values: IInitialValues) => {
    ...
    const hasEmailMatch = database.some(user => user.email.includes(formData.email));
    console.log('formData', formData);
    console.log('hasEmailMatch', hasEmailMatch);
      ...
      >
        ...
        <Box sx={style}>
            <Grid container spacing={2}>
                ...
                <Grid item xs={12}>
                    <FormControl fullWidth error={formik.touched.number && Boolean(formik.errors.number)}>
                ...
                <Button variant="contained" type="submit" style={{ marginTop: 20 }}>Регистрация</Button>
            </Grid>
        </Box>
        </form>
        </Formik>
