...
interface Props {
  ...
  onChange: (chosenClothes: string[]) => void;
}

const Clothes = ({ chosenClothes, onChange }: Props) => {
  ...
  const handleItemClick = (itemId: string) => {
    ...
    if (selectedItem) {
      let updatedChosenClothes;
      if (chosenClothes.includes(itemId)) {
        updatedChosenClothes = chosenClothes.filter((id) => id !== itemId);
      } else {
        updatedChosenClothes = [...chosenClothes, itemId];
      }
      onChange(updatedChosenClothes);
    }
  };
  ...
  return (
    <LayoutBlock>
      ...
      <div className="flex mt-2">
        {items.map((item) => {
          const isSelected = chosenClothes.includes(item.id);
